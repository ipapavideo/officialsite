...
function ajax_plugin_channel_subscribe()
{
    ...
    if ($action == 'add') {
		if ($smodel->exists($channel_id, $user_id)) {
  		...
  		$data['code']			= '<button id="subscribe-del" class="btn btn-xs btn-primary rounded-pill btn-subscribe" data-action="del" data-toggle="tooltip" data-placement="top" title="'.__('channel-unsubscribe-help').'"><i class="fa fa-minus"></i> '.__('unsubscribe').' ('.$subscribers.')</button>';
  		...
  		$data['code']   		= '<button id="subscribe-add" class="btn btn-xs btn-primary rounded-pill btn-subscribe" data-action="add" data-toggle="tooltip" data-placement="top" title="'.__('channel-subscribe-help').'"><i class="fa fa-plus"></i> '.__('subscribe').' ('.$subscribers.')</button>';
  		$data['subscribers']	= $subscribers;
  		$data['status'] 		= 1;    
    } else {
