...
(function() {
  window.hola_player({
	...
	'sources': [
	<?php if ($hls and $this->video['hls']): ?>{'src':'<?php echo VHelper_video_hls::get($this->video_id, $this->video['server_id'], 'playlist.m3u8'); ?>', 'type':'application/x-mpegurl'},<?php endif; ?>
	...
	<?php if ($this->vast): if ($this->vast['vast_text']): $vast = "'adsResponse': '".str_replace("\n", '', $this->vast['vast_text'])."'"; else: $vast = "'adTagUrl': '".$this->vast['vast_url']."'"; endif; ?>
	,'ads': {<?php echo $vast; ?>}
	<?php endif; if ($logo = VCfg::get('player.hola.logo_url')): ?>
