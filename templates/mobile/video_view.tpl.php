<?php defined('_VALID') or die('Restricted Access!'); $this->_js[] = '_video_view.js'; ?>
	  <div id="video" data-id="<?php echo $this->video['video_id']; ?>" data-user-id="<?php echo $this->user_id; ?>">
		...
		<div class="left">
		  ...
		  <div class="content-group"><?php echo $adv; ?></div>
		  <?php endif; if ($this->video['channel_id'] != '0' and VCfg::get('video.view_channel')): ?>
		  ...
		  <div class="content-group content-group-light">
			...
			<div class="content-group-right">
			  <ul class="buttons content-tabs">
				...
				<?php if (VCfg::get('video.download') && $this->video['embed_code'] == '' and $this->video['allow_download']): ?><li><a href="#download" class="video-tab" data-tab="download" data-toggle="tooltip" data-placement="top" title="<?php echo __('download-this-video'); ?>"><i class="fa fa-download"></i> <?php echo __('download'); ?></a></li><?php endif; ?>
		  ...
		  <div id="content-tab-about" class="content-group content-tab">
			...
			<div class="content-group-right">
			  ...
			  <?php endif; if (VCfg::get('video.view_tags')): ?>
              <div class="content-info">
                <?php echo __('tags'); ?>: <?php $tags = explode(',', $this->video['tags']); foreach ($tags as $index => $tag): ?>
                <a href="<?php echo video_url().'/tag/'.str_replace(' ', '-', $tag); ?>/"><?php echo e($tag); ?></a><?php if (isset($tags[$index+1])): echo ', '; endif; ?>
                <?php endforeach ;?>
              </div>
			  ...
			  <?php if ($this->video['description'] && VCfg::get('video.view_desc')): ?>
			  <div class="content-info">
				<span><?php echo nl2br(e($this->video['description'])); ?></span>
			  </div>
			  <?php endif; ?>
			</div>
			<div class="clearfix"></div>
		  </div>
		  <?php if (VCfg::get('video.download') && $this->video['embed_code'] == '' and $this->video['allow_download']): ?>
		  <div id="content-tab-download" class="content-group text-center content-tab" style="display: none;">
			<?php if ($this->download === true): if ($this->friends): foreach ($this->files as $file): $url = ($file['url']) ? $file['url'] : video_url().'/download/'.$this->video['video_id'].'/'.$file['file_id'].'/'.$file['resolution'].'/'; ?>
			...
			<div class="alert alert-warning content-group"><?php echo __('video-download-login', array('<a href="'.REL_URL.LANG.'/user/login/" class="login"><span>'.__('login').'</span></a>', '<a href="'.REL_URL.LANG.'/user/signup/"><span>'.__('signup').'</span></a>')); ?></div>
			<?php else: ?>
			<div class="alert alert-danger content-group"><?php echo $this->download; ?></div>
			<?php endif; ?>
		  </div>
		  <?php endif; ?>
		  <div id="content-tab-share" class="content-group content-tab" style="display: none;">
			<div class="row">
			  ...
			  <div class="col-sm-12">
				...
				<input type="text" class="form-control autoselect" value='<iframe src="<?php echo BASE_URL,'/embed/',$this->video_id; ?>/" frameborder="0" border="0" scrolling="no" width="100%" height="100%" allowfullscreen></iframe>' readonly>
			  </div>
			  <?php endif; ?>
			</div>
		  </div>
		  ...
		  <div id="comments-body" class="content-group" style="display: none;">
			...
			<?php $this->comments_per_page = VCfg::get('video.comments_per_page'); $this->reply = VCfg::get('video.comment_replies'); $this->vote = VCfg::get('video.comment_vote'); echo $this->fetch('_comment_list'); ?>
		  </div>
		  <?php endif; ?>
		</div>
		</div>
		...
		<div class="tablist">
		  <ul class="nav nav-tabs" role="tablist">
			...
			<?php if (VCfg::get('video.channel_videos') && $this->video['channel_name']): ?><li role="presentation"><a href="#channel-videos" data-url="video_channel_videos&id=<?php echo $this->video_id; ?>" data-target="#channel-videos" aria-controls="channel-videos" role="tab" data-toggle="tabajax"><?php echo __('channel-videos', array($this->video['channel_name'])); ?></a></li><?php endif; ?>
			<?php if (VCfg::get('video.user_videos')): ?><li role="presentation"><a href="#user-videos" data-url="video_user_videos&id=<?php echo $this->video_id; ?>" data-target="#user-videos" aria-controls="user-videos" role="tab" data-toggle="tabajax"><?php echo __('user-videos', array($this->video['username'])); ?></a></li><?php endif; ?>
			<li role="presentation"><a href="#thumbs" aria-controls="thumbs" role="tab" data-toggle="tab"><?php echo __('thumbs'); ?></a></li>
		  </ul>
		  <div class="tab-content">
			<div role="tabpanel" class="tab-pane active" id="related">
			  <?php echo p('adv', 'video-related-native', false, 'adv-native'); ?>
			  <?php if ($this->related): $this->vclass = 'related'; $this->videos = $this->related; $this->id = '-related'; echo $this->fetch('_video_list'); ?>
			  <?php if ($this->related_total > VCfg::get('video.view_per_page')): ?>
			...
			<?php if (VCfg::get('video.channel_videos') && $this->video['channel_name']): ?><div role="tabpanel" class="tab-pane" id="channel-videos"></div><?php endif; ?>
			<?php if (VCfg::get('video.user_videos')): ?><div role="tabpanel" class="tab-pane" id="user-videos"></div><?php endif; ?>
