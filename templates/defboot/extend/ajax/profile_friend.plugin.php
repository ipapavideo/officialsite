...
function ajax_plugin_profile_friend()
{
    ...
    if ($action == 'add') {
        ...
        if (VCfg::get('user.activity')) {
  		...
  		$data['code']		= '<button id="friend-del" class="btn btn-ns btn-menu btn-submit btn-friend" data-action="del"><i class="fa fa-user-times"></i> '.__('unfriend').'</button>';
        ...
        if (VCfg::get('user.activity')) {
  		...
  		$data['code']   	= '<button id="friend-add" class="btn btn-ns btn-submit btn-friend"><i class="fa fa-user-plus"></i> '.__('add-friend').'</button>';
