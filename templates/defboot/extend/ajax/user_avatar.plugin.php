<?php
function ajax_plugin_user_avatar()
{
    ...
    if (isset($_GET['id'])) {
        ...
        $exts		= explode(',', VCfg::get('user.avatar_allowed_ext'));

        if (!in_array($ext, $exts)) {
            $data['error']  = 103;
            $data['msg']    = 'Invalid extension! Allowed extensions: '.implode(', ', $exts).'.';
        ...
        if (!VValid::image($_FILES['file']['tmp_name'])) {
            $data['error']  = 107;
            $data['msg']    = 'File is not a valid image!';
