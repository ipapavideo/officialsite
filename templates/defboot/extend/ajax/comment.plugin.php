...
function ajax_plugin_comment()
{
	...
	if ($type == 'video') {
		...
		$content_url	= model_view($content['slug'], true);
	} elseif ($type == 'wall') {
		$content_url	= BASE_URL.'/users/'.$content['username'].'/';
	}
	...
	$code		= array();
	$username	= ($user_id) ? VSession::get('username') : 'anonymous';
	$vote		= VCfg::get($module.'.comment_vote');
	$login		= ($vote == '1' and !$user_id) ? 'login ' : '';
	$element	= ($parent_id) ? 'div' : 'li';
	$width		= ($parent_id) ? 50 : 64;
	
	$code[]	= '<'.$element.' id="comment-'.$comment_id.'" data-id="'.$comment_id.'" class="media">';
    $code[]	= '<div class="media-left">';
	$code[] = '<a href="'.BASE_URL.LANG.'/users/'.$username.'/" rel="nofollow">';
	$code[] = '<img src="'.USER_URL.'/'.avatar($user_id).'" alt="'.__('username-avatar', array($username)).'" width="'.$width.'" class="media-object">';
    $code[]   = '</a>';
    $code[]   = '</div>';
    ...
    if ($user_id) {
        $code[]   = '<h4 class="pull-left"><a href="'.BASE_URL.LANG.'" rel="nofollow">'.$username.'</a></h4>';
    } else {
        $code[]   = '<h4 class="pull-left">'.$username.'</h4>';
    }

    $code[]   = '<small><i class="fa fa-clock-o"></i> '.__('now').'</small>';
    $code[]   = '<div class="media-buttons pull-right">';
    $code[]   = '<span id="comment-spam-'.$comment_id.'">';
    $code[]   = '<button class="comment-spam btn btn-ns btn-warning" data-id="'.$comment_id.'" data-parent-id="'.$content_id.'">'.__('spam').'</button>';
    $code[]   = '</span>';
    $code[]   = '</div>';
    $code[]   = '<div class="clearfix"></div>';
    $code[]   = '</div>';
    $code[]   = '<p>'.nl2br(e($comment)).'</p>';

    if ($vote) {
  		$code[] = '<div id="comment-footer-'.$comment_id.'" class="media-footer">';
  		$code[] = '<span class="text-success">0</span>';
  		$code[] = '<button class="'.$login.' comment-like btn btn-rate" data-type="'.$type.'" data-id="'.$comment_id.'" data-toggle="tooltip" data-placement="top" title="'.__('vote-up').'"><i class="fa fa-thumbs-up"></i></button>';
  		$code[]	= '<button class="'.$login.' comment-dislike btn btn-rate" data-type="'.$type.'" data-id="'.$comment_id.'" data-toggle="tooltip" data-placement="top" title="'.__('vote-down').'"><i class="fa fa-thumbs-down"></i></button>';
