...
function ajax_plugin_playlist_create()
{
	...
	if (isset($_POST['type']) and isset($_POST['video_id'])) {
		...
		$output[]	= '</div>';
		$output[]	= '<label class="radio-inline"><input name="type" id="type" class="radio" value="0" type="radio" checked="checked"> '.__('public').'</label>';
		$output[]	= '<label class="radio-inline"><input name="type" id="type" class="radio" value="1" type="radio"> '.__('private').'</label>';
        $output[]	= '</form>';
        $output[]   = '</div>';
        $output[]   = '<div class="modal-footer">';
        $output[]   = '<button id="playlist-create" data-id="'.$video_id.'" type="button" class="btn btn-submit btn-block">'.__('create-playlist').'</button>';
        $output[]   = '</div>';
        $output[]   = '</div></div></div>';
