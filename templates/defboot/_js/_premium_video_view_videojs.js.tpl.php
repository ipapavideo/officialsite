...
var player = videojs('player', {
  ...
  'sources': [
  <?php if ($hls): ?>{'src':'<?php echo VHelper_video_hls::get($this->video_id, $this->video['server_id'], 'playlist.m3u8', null, true, $this->trailer); ?>', 'type':'application/x-mpegurl', 'withCredentials': false},<?php endif; ?>
  <?php if (!$hls or !$mobile): foreach ($this->files as $index => $file): $url = ($file['url'] != '') ? $file['url'] : VHelper_video_stream::get_urle($this->video_id, $file['file_id'], $file['postfix'], $file['ext'], $this->video['server_id'], true, $this->trailer); ?>
...
player.on('pause', function() {$('#player-advertising').show();<?php if ($this->trailer): ?> $('#player-premium').show();<?php endif; ?>});
player.on('play', function() {$('#player-advertising').hide();<?php if ($this->trailer): ?> $('#player-premium').hide();<?php endif; ?>});
<?php if ($this->trailer): ?>
