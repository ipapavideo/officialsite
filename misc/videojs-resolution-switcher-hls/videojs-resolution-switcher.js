  ...
  var ResolutionMenuItem = videojs.extend(MenuItem, {
    ...
    videoJsResolutionSwitcher = function(options) {
      ...
      player.updateSrc = function(src, _options){
		...
		// Remove hls urls from the sources
		for (var key in src) {
			var type = src[key].type;
			if (type.toLowerCase() == 'application/x-mpegurl') {
			  src.splice(key, 1);
			}
		}
      ...
      /**
      * Returns sources array without the hls sources
      */
      ...
      function chooseSrc(groupedSrc, src){
        ...
        if(!hls) { return; }
			...
			player.ready(function(){
				...
				} else if (player.options_.sources.length > 1){
					// tech: Html5 and Flash
					// Create resolution switcher for videos form <source> tag inside <video>
					player.updateSrc(player.options_.sources);
				} else if (player.techName_ === 'Youtube'){	
					// tech: YouTube
