...
return menuItems;}});videoJsResolutionSwitcher=function(options){var settings=videojs.mergeOptions(defaults,options),player=this,label=document.createElement('span'),groupedSrc={};videojs.addClass(label,'vjs-resolution-button-label');player.updateSrc=function(src,_options){if(!src){return player.src();}
...
if(_options&&_options.hls){player.src(src);return initResolutionForHLS(player);}
for(var key in src){var type=src[key].type;if(type.toLowerCase()=='application/x-mpegurl'){src.splice(key,1);}}
...
function initResolutionForHLS(player){var hls=player.tech_.hls;if(!hls){return;}
player.on('mediachange',function(){player.trigger('resolutionchange');});player.one('loadedmetadata',function(){var playlists=hls.playlists.master.playlists;var _sources=[{src:'auto',type:'application/x-mpegurl',label:'auto',res:'auto'}];playlists.map(function(value){_sources.push({src:value.uri,type:'application/x-mpegurl',label:value.attributes.NAME||value.attributes.RESOLUTION.height,res:value.attributes.RESOLUTION.height});});groupedSrc=bucketSources(_sources);var selectPlaylist=function(selectedResolution){var _selectPlaylist;if(selectedResolution==='auto'){_selectPlaylist=videojs.HlsHandler.prototype.selectPlaylist;return _selectPlaylist;}
_selectPlaylist=function(){return hls.playlists.master.playlists.filter(function(playlist){return playlist.attributes.RESOLUTION.height===selectedResolution;})[0];};return _selectPlaylist;};var _customSourcePicker=function(_player,_sources,_label){var selectedRes=_sources[0].res;hls.selectPlaylist=selectPlaylist(selectedRes);return player;};var currentPlaylist=hls.playlists.media();var choosen={label:'auto',res:currentPlaylist.attributes.RESOLUTION.height,sources:currentPlaylist};var menuButton=new ResolutionMenuButton(player,{sources:groupedSrc,initialySelectedLabel:choosen.label,initialySelectedRes:choosen.res,customSourcePicker:_customSourcePicker},settings,label);menuButton.el().classList.add('vjs-resolution-button');player.controlBar.resolutionSwitcher=player.controlBar.addChild(menuButton);});}
player.ready(function(){if(player.tech_.hls&&player.options_.sources.length){initResolutionForHLS(player);}else if(player.options_.sources.length>1){player.updateSrc(player.options_.sources);}else if(player.techName_==='Youtube'){initResolutionForYt(player);}});};videojs.plugin('videoJsResolutionSwitcher',videoJsResolutionSwitcher);})(window,videojs);})();(function(){var defaults={file:'Owned_Stamp.png',xpos:0,ypos:0,xrepeat:0,opacity:100,url:''},extend=function(){var args,target,i,object,property;args=Array.prototype.slice.call(arguments);target=args.shift()||{};for(i in args){object=args[i];for(property in object){if(object.hasOwnProperty(property)){if(typeof object[property]==='object'){target[property]=extend(target[property],object[property]);}else{target[property]=object[property];}}}}
return target;};videojs.plugin('watermark',function(options){var settings,video,div,img;settings=extend(defaults,options);video=this.el();div=document.createElement('div');lnk=document.createElement('a');lnk.href=options.url;div.appendChild(lnk);img=document.createElement('img');lnk.appendChild(img);img.className='vjs-watermark';img.src=options.file;if((options.ypos==0)&&(options.xpos==0))
{img.style.top="0";img.style.left="0";}
else if((options.ypos==0)&&(options.xpos==100))
{img.style.top="0";img.style.right="0";}
else if((options.ypos==100)&&(options.xpos==100))
{img.style.bottom="25px";img.style.right="5px";}
else if((options.ypos==100)&&(options.xpos==0))
{img.style.bottom="25px";img.style.left="5px";}
else if((options.ypos==50)&&(options.xpos==50))
{img.style.top=(this.height()/2)+"px";img.style.left=(this.width()/2)+"px";}
div.style.opacity=options.opacity;video.appendChild(div);});})();(function(){var defaults={width:0,height:0,basePath:""},extend=function(){var args,target,i,object,property;args=Array.prototype.slice.call(arguments);target=args.shift()||{};for(i in args){object=args[i];for(property in object){if(object.hasOwnProperty(property)){if(typeof object[property]==='object'){target[property]=extend(target[property],object[property]);}else{target[property]=object[property];}}}}
...
return el;},getScrollOffset=function(){if(window.pageXOffset){return{x:window.pageXOffset,y:window.pageYOffset};}
return{x:document.documentElement.scrollLeft,y:document.documentElement.scrollTop};},parseImageLink=function(imglocation){var lsrc,clip,hashindex,hashstring;hashindex=imglocation.indexOf('#');if(hashindex===-1){return{src:imglocation,w:0,h:0,x:0,y:0};}
lsrc=imglocation.substring(0,hashindex);hashstring=imglocation.substring(hashindex+1);if(hashstring.substring(0,5)!=='xywh='){return{src:defaults.basePath+lsrc,w:0,h:0,x:0,y:0};}
var data=hashstring.substring(5).split(',');return{src:defaults.basePath+lsrc,w:parseInt(data[2]),h:parseInt(data[3]),x:parseInt(data[0]),y:parseInt(data[1])};};videojs.plugin('thumbnails',function(options){var div,settings,img,player,progressControl,duration,moveListener,moveCancel,thumbTrack;defaults.basePath=options.basePath||defaults.basePath;settings=extend({},defaults,options);player=this;var numtracks=player.textTracks().length;if(numtracks===0){return;}
i=0;while(i<numtracks){if(player.textTracks()[i].kind==='metadata'){thumbTrack=player.textTracks()[i];thumbTrack.mode='hidden';break;}
i++;}
...
/*
   ...
   */
if(typeof define==='function'&&define['amd']){define(['./video'],function(vjs){factory(window,document,vjs)});}else if(typeof exports==='object'&&typeof module==='object'){factory(window,document,require('video.js'));}else{factory(window,document,videojs);}})(function(window,document,vjs){var
getCookieItem=function(sKey){if(!sKey||!hasCookieItem(sKey)){return null;}
var reg_ex=new RegExp("(?:^|.*;\\s*)"+
window.escape(sKey).replace(/[\-\.\+\*]/g,"\\$&")+"\\s*\\=\\s*((?:[^;](?!;))*[^;]?).*");return window.unescape(document.cookie.replace(reg_ex,"$1"));},setCookieItem=function(sKey,sValue,vEnd,sPath,sDomain,bSecure){if(!sKey||/^(?:expires|max\-age|path|domain|secure)$/i.test(sKey)){return;}
